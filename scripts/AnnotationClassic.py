...
class AnnotationClassic:
    def __init__(self, annotation_file_name, path_annotations, path_audio, file_type, audio_extension):
    ...
    def read_audio_file(self, file_name, audio_folder_parent):
        '''
        file_name: string, name of file including extension, e.g. "audio1.wav"

        '''
        # Get the path to the file
        audio_folder = os.path.join(audio_folder_parent,file_name)
    ...
    def get_annotation_information(self):
        ...
        if self.file_type == 'svl':
            ...
            xmldoc = minidom.parse(self.path_annotations+self.annotation_file_name+'.svl')
            ...
            if (len(itemlist) > 0):
                ...
                audio_amps, original_sample_rate = self.read_audio_file(file_name_no_extension+self.audio_extension ,self.path_audio)
        ...
        if self.file_type == 'raven_caovitgibbons':
            df = pd.read_csv(os.path.join(self.path_annotations, self.annotation_file_name), sep='\t')
